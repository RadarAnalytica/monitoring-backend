...
from server.funcs.get_product_query_data import get_product_query_payload, get_product_query_payload_v2
...
async def get_product_queries(
    ...
    logger.info(f"Время выполнения v1 {(datetime.now() - start).microseconds / 1000}s")
    return result
...
async def get_product_queries_v2(
    ...
    logger.info(f"Время выполнения v2 {(datetime.now() - start).microseconds / 1000}s")
    return result
