...
from server.funcs.get_product_query_data import get_product_query_payload, get_product_db_data
...
async def get_product_queries(
    ...
    logger.info(f"Время выполнения v1 {(datetime.now() - start).total_seconds()}s")
    return result
...
async def get_product_queries_v2(
    ...
    if not check_jwt_token(token):
        return JSONResponse(status_code=403, content="Unauthorized")
    start = datetime.now()
    result = await get_product_db_data()
